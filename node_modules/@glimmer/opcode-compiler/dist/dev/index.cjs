...
const isGetFreeOptionalHelper = makeResolutionTypeVerifier(wireFormat.SexpOpcodes.GetFreeAsHelperHeadOrThisFallback);
function isGetFreeDeprecatedHelper(opcode) {
  return Array.isArray(opcode) && opcode[0] === wireFormat.SexpOpcodes.GetFreeAsDeprecatedHelperHeadOrThisFallback;
}
const isGetFreeOptionalComponentOrHelper = makeResolutionTypeVerifier(wireFormat.SexpOpcodes.GetFreeAsComponentOrHelperHeadOrThisFallback);
...
function resolveOptionalHelper(resolver, constants, meta, _ref5) {
  ...
  util.assert(isGetFreeOptionalHelper(expr) || isGetFreeDeprecatedHelper(expr), 'Attempted to resolve a helper with incorrect opcode');
  let {
    upvars,
    owner
  } = assertResolverInvariants(meta);
...
function resolveOptionalComponentOrHelper(resolver, constants, meta, _ref6) {
  ...
  util.assert(isGetFreeOptionalComponentOrHelper(expr), 'Attempted to resolve an optional component or helper with incorrect opcode');
  let type = expr[0];
  if (type === wireFormat.SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues,
...
class Labels {
  constructor() {
    _defineProperty(this, "labels", util.dict());
  ...
  if (isBuilderOpcode(op[0])) {
    ...
    switch (op[0]) {
      ...
      case HighLevelResolutionOpcodes.TemplateLocal:
        {
          ...
          let value = util.expect(meta.scopeValues, 'BUG: Attempted to gect a template local, but template does not have any')[valueIndex];
      ...
      case HighLevelResolutionOpcodes.Free:
        if (env.DEBUG) {
          let [, upvarIndex] = op;
          let freeName = util.expect(meta.upvars, 'BUG: attempted to resolve value but no upvars found')[upvarIndex];
...
class EncoderImpl {
  constructor(heap, meta, stdlib) {
    _defineProperty(this, "labelsStack", new util.Stack());
    _defineProperty(this, "encoder", new encoder.InstructionEncoderImpl([]));
  ...
  push(constants, type) {
    ...
    let first = type | machine | (arguments.length <= 2 ? 0 : arguments.length - 2) << vm.ARG_SHIFT;
    heap.pushRaw(first);
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.Concat, (op, _ref) => {
  let [, parts] = _ref;
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.Call, (op, _ref2) => {
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.Curry, (op, _ref3) => {
  let [, expr, type, positional, named] = _ref3;
  Curry(op, type, expr, positional, named);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetSymbol, (op, _ref4) => {
  let [, sym, path] = _ref4;
  op(vm.Op.GetVariable, sym);
  withPath(op, path);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetLexicalSymbol, (op, _ref5) => {
  let [, sym, path] = _ref5;
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetStrictKeyword, (op, _ref6) => {
  let [, sym, _path] = _ref6;
  op(HighLevelResolutionOpcodes.Free, sym, _handle => {
    // TODO: Implement in strict mode
  });
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetFreeAsComponentOrHelperHeadOrThisFallback, () => {
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetFreeAsHelperHeadOrThisFallback, (op, expr) => {
  // <div id={{baz}}>
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetFreeAsDeprecatedHelperHeadOrThisFallback, (op, expr) => {
  // <Foo @bar={{baz}}>
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.HasBlock, (op, _ref7) => {
  let [, block] = _ref7;
  expr(op, block);
  op(vm.Op.HasBlock);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.HasBlockParams, (op, _ref8) => {
  let [, block] = _ref8;
  expr(op, block);
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.IfInline, (op, _ref9) => {
...
EXPRESSIONS.add(wireFormat.SexpOpcodes.Not, (op, _ref10) => {
  let [, value] = _ref10;
  expr(op, value);
  op(vm.Op.Not);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.GetDynamicVar, (op, _ref11) => {
  let [, expression] = _ref11;
  expr(op, expression);
  op(vm.Op.GetDynamicVar);
});
EXPRESSIONS.add(wireFormat.SexpOpcodes.Log, (op, _ref12) => {
...
function meta(layout) {
  ...
  return {
    ...
    upvars: upvars,
    scopeValues: ((_layout$scope = layout.scope) === null || _layout$scope === void 0 ? void 0 : _layout$scope.call(layout)) ?? null,
...
STATEMENTS.add(wireFormat.SexpOpcodes.Modifier, (op, _ref) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.Yield, (op, _ref11) => {
  let [, to, params] = _ref11;
  return YieldBlock(op, to, params);
});
STATEMENTS.add(wireFormat.SexpOpcodes.AttrSplat, (op, _ref12) => {
  let [, to] = _ref12;
  return YieldBlock(op, to, null);
});
STATEMENTS.add(wireFormat.SexpOpcodes.Debugger, (op, _ref13) => {
  let [, debugInfo] = _ref13;
  return op(vm.Op.Debugger, debugSymbolsOperand(), debugInfo);
});
STATEMENTS.add(wireFormat.SexpOpcodes.Append, (op, _ref14) => {
  let [, value] = _ref14;
...
STATEMENTS.add(wireFormat.SexpOpcodes.TrustingAppend, (op, _ref15) => {
  let [, value] = _ref15;
  if (!Array.isArray(value)) {
    op(vm.Op.Text, value === null || value === undefined ? '' : String(value));
  } else {
...
STATEMENTS.add(wireFormat.SexpOpcodes.Block, (op, _ref16) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.InElement, (op, _ref17) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.If, (op, _ref18) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.Each, (op, _ref19) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.With, (op, _ref20) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.Let, (op, _ref21) => {
  let [, positional, block] = _ref21;
  let count = CompilePositional(op, positional);
  InvokeStaticBlockWithStack(op, block, count);
});
STATEMENTS.add(wireFormat.SexpOpcodes.WithDynamicVars, (op, _ref22) => {
...
STATEMENTS.add(wireFormat.SexpOpcodes.InvokeComponent, (op, _ref23) => {
