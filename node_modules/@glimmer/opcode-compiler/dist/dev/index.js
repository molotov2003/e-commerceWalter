...
import { assert as assert$1, deprecate } from '@glimmer/global-context';
...
function resolveOptionalHelper(resolver, constants, meta, _ref5) {
  ...
  assert(isGetFreeOptionalHelper(expr) || isGetFreeDeprecatedHelper(expr), 'Attempted to resolve a helper with incorrect opcode');
  let {
    upvars,
    owner
  } = assertResolverInvariants(meta);
...
function resolveOptionalComponentOrHelper(resolver, constants, meta, _ref6) {
  ...
  assert(isGetFreeOptionalComponentOrHelper(expr), 'Attempted to resolve an optional component or helper with incorrect opcode');
  let type = expr[0];
  if (type === SexpOpcodes.GetLexicalSymbol) {
    let {
      scopeValues,
  ...
  op(HighLevelResolutionOpcodes.Local, expr[1], _name => {
    op(HighLevelResolutionOpcodes.OptionalHelper, expr, {
      ifHelper: (handle, name, moduleName) => {
        assert$1(expr[2] && expr[2].length === 1, '[BUG] Missing argument name');
        let arg = expr[2][0];
