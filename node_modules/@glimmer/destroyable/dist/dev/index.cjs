...
function destroy(destroyable) {
  ...
  iterate(destructors, destructor => globalContext.scheduleDestroy(destroyable, destructor));
  globalContext.scheduleDestroyed(() => {
...
exports.enableDestroyableTracking = void 0;
exports.assertDestroyablesDestroyed = void 0;
if (env.DEBUG) {
  let isTesting = false;
  exports.enableDestroyableTracking = () => {
    if (isTesting) {
  ...
  exports.assertDestroyablesDestroyed = () => {
    if (!isTesting) {
    ...
    if (undestroyed.length > 0) {
      let objectsToString = undestroyed.map(util.debugToString).join('\n    ');
