import { assertNever, dict, expect, isPresentArray, values, assert, NS_XMLNS, NS_XML, NS_XLINK, exhausted, mapPresentArray, unreachable, getLast, assertPresentArray } from '@glimmer/util';
import { VariableResolutionContext, SexpOpcodes, WellKnownTagNames, WellKnownAttrNames } from '@glimmer/wire-format';
import { node, generateSyntaxError, ASTv2, KEYWORDS_TYPES, isKeyword, SourceSlice, src, maybeLoc, normalize as normalize$1 } from '@glimmer/syntax';
    ...
    case HeadKind.Call:
      {
        ...
        let builtExpr = buildCallHead(path, trusted ? VariableResolutionContext.AmbiguousInvoke : VariableResolutionContext.AmbiguousAppendInvoke, symbols);
...
function buildKeyword(normalized, symbols) {
  ...
  switch (name) {
    case 'with':
      return [SexpOpcodes.With, expect(params, 'with requires params')[0], block, inverse];
    case 'if':
...
class With extends node('With').fields() {}
...
class EndBlock extends node('EndBlock').fields() {}
...
class DeprecatedCallExpression extends node('DeprecatedCallExpression').fields() {}
...
class GetWithResolver extends node('GetWithResolver').fields() {}
class GetSymbol extends node('GetSymbol').fields() {}
class GetFreeWithContext extends node('GetFreeWithContext').fields() {}
/** strict mode */
class GetFree extends node('GetFree').fields() {}
  ...
  formatOpcode(opcode) {
    if (Array.isArray(opcode)) {
      switch (opcode[0]) {
        ...
        case SexpOpcodes.GetFreeAsDeprecatedHelperHeadOrThisFallback:
...
class ContentEncoder {
  ...
  With(_ref14) {
    ...
    return [SexpOpcodes.With, EXPR.expr(value), CONTENT.NamedBlock(block)[1], inverse ? CONTENT.NamedBlock(inverse)[1] : null];
  }
  Let(_ref15) {
    let {
      positional,
